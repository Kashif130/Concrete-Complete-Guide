...
function buildShareText(v: VaultResult, t: TFn): string {
  const lines = [
    `${t("trVpShareMy", { name: v.name ?? t("trVpShareGeneric") })}${
      v.symbol ? ` (${v.symbol})` : ""
    } 🏗️🗿`,
    `${round(v.underlyingValueFormatted)} ${v.underlyingSymbol ?? ""}`.trim(),
  ];
  if (v.sharePrice !== null) {
    lines.push(t("trVpSharePriceShare", { p: v.sharePrice.toFixed(6) }));
  }
  if (v.chain) lines.push(t("trVpShareChain", { c: v.chain }));
  lines.push(t("trVpShareVault", { a: truncate(v.vaultAddress) }));
...
function VaultRow({ v, wallet }: { v: VaultResult; wallet?: string }) {
  const t = useVT();
    ...
    <div className="grid gap-3 px-6 py-5 sm:grid-cols-[1.2fr_1fr_1fr_1fr]">
        ...
        <>
          <div>
            ...
            <div className="font-mono text-xs text-inkMuted">
              {t("trVpShares", { n: round(v.sharesHeldFormatted) })}
            </div>
            {positionDiff !== null && (
              <div
              ...
              >
                {t("trVpSinceVisit", {
                  pct: `${positionDiff >= 0 ? "+" : ""}${positionDiff.toFixed(2)}`,
                })}
              </div>
            )}
          </div>
          ...
          <div>
            ...
            {apy !== null && (
              <div className="mt-1 font-mono text-[11px] text-inkMuted">
                {t("trVpApy", { pct: (apy * 100).toFixed(1) })}
              </div>
            )}
            <ShareButtons text={buildShareText(v, t)} />
          </div>
        </>
      )}
      ...
      {!v.error && wallet && (
        <PositionLedger
          ...
          currentValue={Number(v.underlyingValueFormatted) || 0}
    ...
    <section className="border border-concreteMuted/40 bg-surface">
      <header className="border-b border-concreteMuted/40 px-6 py-4">
        <h2 className="text-lg text-ink">
          {t("trVpTitle")}{walletLabel ? ` — ${walletLabel}` : ""}
        </h2>
      ...
      <div className="divide-y divide-concreteMuted/30">
        {vaults.map((v) => (
          <VaultRow key={`${v.chain ?? ""}-${v.vaultAddress}`} v={v} wallet={wallet} />
        ))}
      </div>
      ...
      {okVaults.length > 0 && topHolding && (
        <div className="border-t border-concreteMuted/30 px-6 py-4">
          <ShareCardImage
            ...
            headlineLabel={topHolding.name ?? t("trVpFallback")}
            wallet={wallet}
