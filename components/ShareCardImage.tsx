...
type Props = {
  ...
  filename: string;
  /** Tier-unlock framing: swaps the top-left badge for "CERTIFICATE OF ACHIEVEMENT" and stamps today's date in the footer, instead of the generic "PREMIUM" badge. */
  certificate?: boolean;
};
...
export default function ShareCardImage({
  ...
  function draw(): HTMLCanvasElement | null {
    ...
    // --- Badge under the kicker: tier-unlock cards get a certificate
    // stamp instead of the generic premium badge ---
    ctx.font = "600 20px sans-serif";
    ctx.fillStyle = "#C9A227";
    ctx.fillText(certificate ? "🎫 CERTIFICATE OF ACHIEVEMENT" : "🗿 PREMIUM", 70, 116);
    ...
    const footer = certificate
      ? `${SITE_URL.replace(/^https?:\/\//, "")} · awarded ${new Date().toLocaleDateString()} · not built or endorsed by Blueprint Finance`
      : `${SITE_URL.replace(/^https?:\/\//, "")} · not built or endorsed by Blueprint Finance`;
    ctx.fillText(footer, 68, H - 56);
